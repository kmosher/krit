...
A thin [Tauri](https://v2.tauri.app/) shell that gives krit its own macOS app:
each review opens as its own window under a single **krit** dock icon, instead
of a tab in your default browser.

It owns no UI of its own. Every window is pointed at a running krit server
(`http://127.0.0.1:<port>`); the krit CLI hands reviews over with a deep link:

```
krit://review?url=http://127.0.0.1:51234&title=my-repo · my-branch
```

The app claims the `krit://` scheme on first launch. A link that arrives while
it's running opens a new window; a cold start opens the launch link's window.
Windows are keyed by server port, so re-running krit for a review that's still
...
## Wiring krit to the app

Set the launcher in `~/.config/krit/settings.json`:

```json
{ "launcher": "app" }
```

With that, `krit` fires the deep link instead of opening a browser tab. Without
...
# Release bundle → src-tauri/target/release/bundle/macos/krit.app:
cargo tauri build
```

Then move `krit.app` into `/Applications` (and launch it once so macOS
registers the `krit://` scheme).
...
- **No window restore.** Closing a review window doesn't end the review; the
  krit server keeps running until you click *Done reviewing* or Ctrl-C the CLI.
