...
beforeEach(() => {
  ...
  vi.stubGlobal(
    ...
    vi.fn((url: string, init?: { method?: string; body?: string }) => {
      ...
      if (method === 'GET') return Promise.resolve({ json: () => Promise.resolve(server) })

      if (method === 'POST' && url === '/api/comments') {
        const created = comment({ id: `srv-${server.length + 1}`, ...(body as Partial<ReviewComment>) })
        server = [...server, created]
        return Promise.resolve({ json: () => Promise.resolve(created) })
      }
      if (method === 'POST' && url === '/api/drafts/post') {
        ...
        return Promise.resolve({ json: () => Promise.resolve({ ok: true, posted }) })
      }
      if (method === 'POST') {
        ...
        return Promise.resolve({ json: () => Promise.resolve(updated) })
      }
      if (method === 'PUT') {
        const id = url.split('/')[3]
...
describe('adding a comment', () => {
  ...
  it('keeps the comments already on screen when a new one is added', async () => {
    ...
    const { result } = await mount([comment({ id: 'a' }), comment({ id: 'b' })])
    act(() => {
  ...
  it('leaves the existing comments alone when the POST fails', async () => {
    ...
    const { result } = await mount([comment({ id: 'a' })])
    failMethod = 'POST'
    await act(async () => {
...
describe('editing, resolving and deleting', () => {
  it('edits the body in place without touching the other comments', async () => {
    const { result } = await mount([comment({ id: 'a' }), comment({ id: 'b', body: 'other' })])
  ...
  it('resolving sends only a status, so it cannot blank the body', async () => {
    ...
    const { result } = await mount([comment({ id: 'a', body: 'keep me' })])
...
describe('replying', () => {
  it('tags the reply as coming from the browser', async () => {
    ...
    const { result } = await mount([comment({ id: 'a', status: 'resolved' })])
...
describe('posting drafts', () => {
  it('flushes drafts in one request and refetches the flipped statuses', async () => {
    ...
    const { result } = await mount([comment({ id: 'a', status: 'draft' }), comment({ id: 'b', status: 'draft' })])
  ...
  it('counts only drafts', async () => {
    const { result } = await mount([
      comment({ id: 'a', status: 'draft' }),
      comment({ id: 'b', status: 'open' }),
      comment({ id: 'c', status: 'resolved' }),
    ])
...
describe('reacting to a server-side reanchor', () => {
  ...
  it("moves the annotation to the comment's new line", async () => {
    ...
    const { result } = await mount([comment({ id: 'a', lineNumber: 10, endLine: 10 })])
    expect(result.current.getAnnotationsForFile('src/a.rs')[0].lineNumber).toBe(10)

    server = [comment({ id: 'a', lineNumber: 42, endLine: 42 })]
    await poll(result, 1)
  ...
  it('follows the end of a multi-line range that moved', async () => {
    ...
    const { result } = await mount([comment({ id: 'a', lineNumber: 10, endLine: 12 })])
    server = [comment({ id: 'a', lineNumber: 20, endLine: 22 })]
    await poll(result, 1)
  ...
  it('tells the agent when a position is only a best-effort re-anchor', async () => {
    ...
    server = [comment({ id: 'a', lineNumber: 10, endLine: 10, outdated: true })]
    await poll(result, 1)
  ...
  it('clears the outdated flag when the server re-finds the anchor', async () => {
    ...
    const { result } = await mount([comment({ id: 'a', outdated: true })])
    expect(result.current.formatAllComments()).toContain('outdated="true"')
    server = [comment({ id: 'a', outdated: false, lineNumber: 11, endLine: 11 })]
    await poll(result, 1)
    expect(result.current.formatAllComments()).not.toContain('outdated')
  })
  ...
  it('keeps an outdated comment visible and commentable', async () => {
    ...
    const { result } = await mount([comment({ id: 'a', outdated: true, status: 'open' })])
    expect(result.current.comments).toHaveLength(1)
...
describe('annotations', () => {
  it("returns only the requested file's comments", async () => {
    ...
    const { result } = await mount([
      comment({ id: 'a', filePath: 'src/a.rs' }),
      comment({ id: 'b', filePath: 'src/b.rs' }),
    ])
  ...
  it('falls back to lineNumber when endLine is absent', async () => {
    ...
    const { result } = await mount([comment({ id: 'a', lineNumber: 7, endLine: undefined })])
  ...
  it('keeps the deletion side', async () => {
    ...
    const { result } = await mount([comment({ id: 'a', side: 'deletions' })])
...
describe('formatAllComments', () => {
  it('withholds drafts from the copied text', async () => {
    ...
    const { result } = await mount([
      comment({ id: 'a', status: 'draft', body: 'secret' }),
      comment({ id: 'b', body: 'public' }),
    ])
  ...
  it('produces nothing at all when everything is a draft', async () => {
    ...
    const { result } = await mount([comment({ id: 'a', status: 'draft' })])
    expect(result.current.formatAllComments()).toBe('')
  })

  it('groups comments under one element per file', async () => {
    const { result } = await mount([
      comment({ id: 'a', filePath: 'src/a.rs', body: 'one' }),
      comment({ id: 'b', filePath: 'src/b.rs', body: 'two' }),
      comment({ id: 'c', filePath: 'src/a.rs', body: 'three' }),
    ])
    const out = result.current.formatAllComments()
  ...
  it('prefixes a deletion-side line with -', async () => {
    ...
    const { result } = await mount([comment({ id: 'a', side: 'deletions', lineContent: 'gone()' })])
    expect(result.current.formatAllComments()).toContain('<code>- gone()</code>')
  })

  it('gives a multi-line range one diff line per row', async () => {
    const { result } = await mount([
      comment({ id: 'a', lineNumber: 3, endLine: 5, lineContent: 'one\ntwo\nthree' }),
    ])
    const out = result.current.formatAllComments()
  ...
  it('hands over the exact selected substring alongside the columns', async () => {
    ...
    const { result } = await mount([
      comment({ id: 'a', lineNumber: 3, endLine: 3, startColumn: 4, endColumn: 5, selectedText: 'x' }),
    ])
    const out = result.current.formatAllComments()
  ...
  it('emits a column anchor of zero rather than dropping it', async () => {
    ...
    const { result } = await mount([
      comment({ id: 'a', startColumn: 0, endColumn: 3, selectedText: 'let' }),
    ])
  ...
  it('renders a suggestion as a GitHub-style fence', async () => {
    const { result } = await mount([
      comment({ id: 'a', body: 'simpler', suggestion: { newLines: ['let x = 2;', 'ok()'] } }),
    ])
    const out = result.current.formatAllComments()
...
describe('copyAllComments', () => {
  it('writes the formatted review to the clipboard', async () => {
    ...
    const { result } = await mount([comment({ id: 'a', body: 'copied' })])
    await act(async () => {
